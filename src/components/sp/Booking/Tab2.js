...
import FilterDialog from "./FilterDialog"; // Import the FilterDialog component
  ...
  const handleFilterApply = async (startDate, endDate) => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/filter/filterdone/${providerId}`,
        {
  ...
  const handleClearFilterClick = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/enquiry/fetchdoneenquiry/${providerId}`
      );
      const bookingsWithDetails = await Promise.all(
        response.data.map(async (enquiry) => {
          const EnquiryDetails = await fetchEnquiryDetails(enquiry.enquiryId);
                  ...
                  <Box style={{ padding: "0.5rem" }}>
                    <Box ml={1}>
                      ...
                      <Box mt={1} ml={1}>
                        ...
                        <Box>
                          <strong>state : </strong>{" "}
                          {enquiry.EnquiryDetails.state}
                        </Box>
                        <Box>
                          <strong>city : </strong> {enquiry.EnquiryDetails.city}
                        </Box>
                        <Box>
                        ...
                        <Box>
                          <strong>email : </strong> {enquiry.UserDetails.email}
                        </Box>
                      </Box>
                    </Box>
                    <Box
