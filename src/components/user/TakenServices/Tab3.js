...
const Tab3 = () => {
  ...
  const handleFilterApply = async (startDate, endDate) => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/filter/filterdeclineuser/${userId}`,
        {
  ...
  const handleClearFilterClick = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/enquiry/fetchdeclineenquiryuser/${userId}`
      );
      const bookingsWithDetails = await Promise.all(
        response.data.map(async (enquiry) => {
          // Fetch service provider details
  ...
  return (
    <Box sx={{ padding: "1rem 4rem" }}>
      <h2>Your Enquiries is decline from provider</h2>
      <Box
        sx={{
          display: "flex",
          alignItems: "center",
              ...
              >
                {enquiry.serviceProviderDetails && (
                  <Box style={{ padding: "1rem" }}>
                    ...
                    <Box style={{ display: "flex", alignItems: "center" }}>
                      <Box mt={1.5} variant="body1">
                        <strong>Decline Reason: </strong> {enquiry.reason}
                      </Box>
                    </Box>
