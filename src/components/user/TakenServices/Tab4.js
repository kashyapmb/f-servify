...
const Tab4 = () => {
  ...
  const handleFilterApply = async (startDate, endDate) => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/filter/filterdoneuser/${userId}`,
        {
  ...
  const handleClearFilterClick = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/enquiry/fetchdoneenquiryuser/${userId}`
      );
      const bookingsWithDetails = await Promise.all(
        response.data.map(async (enquiry) => {
          // Fetch service provider details
  ...
  return (
    <Box sx={{ padding: "1rem 4rem" }}>
      <h2>Your Enquiries are taken care by provider</h2>
      <Box
        sx={{
          display: "flex",
          alignItems: "center",
